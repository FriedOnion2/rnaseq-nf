...
| 基因 | 对照样本 count | 处理样本 count |
|---|---|---|
| 基因 X | 1000 | 3000 |
| 其它所有基因合计 | 999,000 | 2,997,000 |

看起来基因 X 翻了三倍（1000 → 3000）。但如果处理样本**整体测得更深**（总 count
是对照的 3 倍），那基因 X 的 3000 里，有 3 倍只是"总量变多了"，**真正的生物学变化是 0**。

归一化做的事：把每个样本按"总深度"缩放到同一水平。缩放后基因 X 变成：

```
对照（总量 100 万）：1000
处理（总量 300 万，除以 3）：3000 / 3 = 1000
→ 归一化后两者相等，基因 X 其实没变！
```

> **一句话**：你数到的 count 同时受"基因表达"和"样本测序深度"两个因素影响，
> 归一化就是先把"深度"这个干扰因素剔除，剩下的才是"真实的表达差异"。
...
### 7.4 多重复样本到底帮了什么忙？（DESeq2 的"含金量"所在）

这是 DESeq2 论文最核心的创新，也是为什么它比早期方法（如 edgeR 的简单版本）更稳。

**问题**：RNA-seq 里，**低表达基因的方差估得极不准**。一个基因只有几百条 count，
样本间随机波动剧烈，你从 3 个重复里估出来的"方差"可能忽大忽小，导致：
- 方差被**低估** → 假阳性暴增（以为差异显著，其实只是运气）
- 方差被**高估** → 漏掉真实的差异

**DESeq2 的解法：dispersion 收缩（shrinkage）**

直觉：如果**所有基因**的方差都从"表达量"这个变量来预测（表达越低、方差越大），
那么一个低表达基因的方差，不该只信它自己那 3 个重复的随机估计，而应该**朝
"全体基因的平均趋势"拉一拉**——这就是"收缩"。

```
某个基因自己估的 dispersion  ↕  观测值（可能不准）
        ↓  收缩（shrink 向全体趋势）
   居中后的 dispersion     ✓  既用了自身信息，又借了全体的力量
```

> **一句话**：当重复数少、基因表达低时，DESeq2 会让它们"互相借力"，
> 用"全体基因的方差-表达量关系"来校准单个基因的方差。这大大减少了假阳性和漏检。

**这就是为什么每组要至少 3 个重复**：重复越多，方差估得越准，shrinkage 需要的
"借力"越少，结果越可靠。3 是底线，6–8 理想。
